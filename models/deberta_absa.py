...
class DebertaForASBA(DebertaForSequenceClassification):
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None, labels=None):
        outputs = self.deberta(input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids, position_ids=position_ids)
        #print(outputs)
...
model = DebertaForASBA.from_pretrained('microsoft/deberta-base', num_aspect_labels=num_aspect_labels, num_polarization_labels=num_polarization_labels)
...
def training_run(model, num_epochs, batch_size, train_dataset, val_dataset):
 ...
 training_aspect, training_polarization, validation_aspect, validation_polarization = training_run(model, 20, 1, train_dataset, val_dataset)
